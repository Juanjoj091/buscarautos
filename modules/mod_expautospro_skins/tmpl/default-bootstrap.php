...
$document->addStyleSheet(JURI::root() . 'modules/mod_expautospro_skins/css/mod_expautospro_skins_bootstrap.css');
...
<div id="modexpskins" class="modexpskins <?php echo $moduleclass_sfx ?>">
    <?php if ($mark_skins): ?>
        <form action="<?php echo JRoute::_(JURI::getInstance()->toString()); ?>#modexpskins" method="post">
            <div class="btn-group">
                <button class="btn btn-info disabled" disabled="disabled"><?php echo JText::_('MOD_EXPAUTOSPRO_SKINS_SELECT_SKIN_TEXT'); ?></button>
                <?php foreach ($mark_skins as $key=>$skin) : ?>
                    <?php if ($skin !== 'default_print'): ?>
                        ...
                        $expactivestyle = '';
                        if ($skin == $expskin) {
                            $expactivestyle = 'current active btn-info disabled';
                        }
                        ?>
                        <button class="<?php echo $expactivestyle; ?> btn" <?php if($expactivestyle): ?> disabled="disabled"<?php endif;?> type="submit" name="expskin" value="<?php echo $skin;?>"><?php echo $skin_link;?></button>
                    <?php endif; ?>
                <?php endforeach; ?>
            </div>

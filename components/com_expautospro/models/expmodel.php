...
class ExpautosproModelExpmodel extends JModelList {
    ...
    function getItem() {
        if (!isset($this->_item)) {
            ...
            if ($this->_item === false) {
                ...
                $query->join('LEFT', '#__expautos_make AS mk ON mk.id = a.makeid');
                //$query->where('mk.state=1');

                /* Categories */
                /*
                $query->select('c.name AS category_name, c.id AS categid');
                $query->join('LEFT', '#__expautos_categories AS c ON c.id = mk.catid');
                */
                $query->select('c.title AS category_name, c.id AS categid');
                $query->join('LEFT', '#__categories AS c ON c.id = mk.catid');
                ...
                $expgroup = $expparams->get('c_admanager_mdpage_groupby');
                $expsort = $expparams->get('c_admanager_mdpage_sortby');

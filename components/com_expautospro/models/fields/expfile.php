...
class JFormFieldExpfile extends JFormField {
    ...
    public function getInput() {
        ...
        $result = ExpAutosProExpparams::getExpParams('admanager', $table_id);
        $result = $result->get('expfile');
        if ($result) {
            $logoimg = '<li><span class="readonly">';
            $logoimg .= '<a href="'.ExpAutosProExpparams::FilesUrlPatch().$result.'" target="_blank">'.$result.'</a>';
            $logoimg .= '</span></li>';
            $logoimg .= '<li>';
            $logoimg .='<label id="jform_delete_photo-lbl" for="jform_delete_photo" title="">';
            $logoimg .= JText::_('COM_EXPAUTOSPRO_CP_USER_DELETEFILE_TEXT');
            $logoimg .='</label>';

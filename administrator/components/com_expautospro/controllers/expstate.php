...
class ExpAutosProControllerExpstate extends JControllerForm {
    ...
    protected function allowAdd($data = array()) {
        ...
        if ($categoryId) {
            $allow = $user->authorise('core.create', $this->option . '.expstate.' . $categoryId);
        }
    ...
    protected function allowEdit($data = array(), $key = 'id') {
        ...
        if ($categoryId) {
            return JFactory::getUser()->authorise('core.edit', $this->option . '.expstate.' . $categoryId);
        } else {
    ...
    public function make_build_contr() {
        ...
        $model = $this->getModel('Expstate');
        $makeaction = JRequest::getString('make_action', 0);
        $buildcat = JRequest::getInt('build_cat', 0);
        $cid = JRequest::getVar('cid', array(), 'post', 'array');

        // Preset the redirect
        $this->setRedirect('index.php?option=com_expautospro&view=expstates');

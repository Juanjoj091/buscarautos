...
class ExpAutosProControllerExpeqcat extends JControllerForm {
    ...
    protected function allowAdd($data = array()) {
        ...
        if ($categoryId) {
            $allow = $user->authorise('core.create', $this->option . '.expeqcat.' . $categoryId);
        }
    ...
    protected function allowEdit($data = array(), $key = 'id') {
        ...
        if ($categoryId) {
            return JFactory::getUser()->authorise('core.edit', $this->option . '.expeqcat.' . $categoryId);
        } else {
    ...
    public function make_build_contr() {
        ...
        $model = $this->getModel('Expeqcat');
        $makeaction = JRequest::getString('make_action', 0);
        $buildcat = JRequest::getInt('build_cat', 0);
        $cid = JRequest::getVar('cid', array(), 'post', 'array');

        // Preset the redirect
        $this->setRedirect('index.php?option=com_expautospro&view=expeqcats');

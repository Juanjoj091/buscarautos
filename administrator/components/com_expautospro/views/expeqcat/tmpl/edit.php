...
<script type="text/javascript">
    Joomla.submitbutton = function(task)
    {
       /* 
        ...
        if (form.jform_catid.value == "0" && task != 'expeqcat.cancel'){
            form.jform_catid.style.backgroundColor = '#FFEAEA';
            form.jform_catid.focus();
            return false;
        }
        */
        if (task == 'expeqcat.cancel' || document.formvalidator.isValid(document.id('expautosprocat-form'))) {
...
<form action="<?php echo JRoute::_('index.php?option=com_expautospro&layout=edit&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="expautosprocat-form" class="form-validate">
    <div class="width-100 fltlft">
        <fieldset class="adminform">
            <legend><?php echo empty($this->item->id) ? JText::_('COM_EXPAUTOSPRO_EQCAT_NEW_TITLE') : JText::sprintf('COM_EXPAUTOSPRO_EQCAT_EDIT_TITLE', $this->item->id); ?></legend>
